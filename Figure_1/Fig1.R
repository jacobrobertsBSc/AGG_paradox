...
#Reading in the Prot.FCC log odds 
data = read.csv("ProtFCC_log_odds_results.csv", h = TRUE)

pdf("figure1.pdf", width = 10, height = 5) #Creating a blank PDF file
fig1 = ggplot(data, aes(x = Codon, y = Log_Odds)) + #Creating a ggplot with the log odds assigned to the Y axis and the codons on the X 
