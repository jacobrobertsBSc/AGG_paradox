...
#define which columns of data i want from the source csv file (a nice pandas feature)
columns = ["Name", "CDS.seq", "Trans.FCC"]
...
q75 = df["Trans.FCC"].quantile(q = 0.75)

high = df[df["Trans.FCC"].ge(q75)]
#repeat for the lower quantile, now asking for less than or equal to (le):
q25 = df["Trans.FCC"].quantile(q = 0.25)

low = df[df["Trans.FCC"].le(q25)]
